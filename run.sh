...
case "$1" in
    ...
    *)
        ...
        export IPADDR="$(terraform show | grep "^ip_address" | awk '{print $3}' | awk -F'"' '{print $2}')"        
        sleep 15
        export STATUS="$(curl -k -o /dev/null -s -w "%{http_code}\n" https://$IPADDR)"
        
        
        if [ $STATUS -eq 302 ]
        then
        	echo -e "\033[01;32m Status do Site\033[0m: UP"
            echo -e "\033[01;32m URL Configuração WordPress\033[0m: https://$IPADDR/wp-admin/"
            echo -e "\033[01;32m URL de Acesso ao WordPress\033[0m: https://$IPADDR"
        else
